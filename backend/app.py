...
def get_questions():
    ...
    company = data.get('company', '')  # Make company optional
...
def create_did_stream():
    ...
    try:
        print(f"[DEBUG] Creating D-ID stream with payload: {payload}")
        ...
        print(f"[DEBUG] D-ID response status: {did_response.status_code}")
        print(f"[DEBUG] D-ID response text: {did_response.text}")
        
        if did_response.status_code == 201:
            return jsonify(did_response.json())
        else:
            error_msg = f"D-ID API returned {did_response.status_code}: {did_response.text}"
            print(f"[ERROR] {error_msg}")
            return jsonify({'error': error_msg}), 500
            
    except requests.exceptions.RequestException as e:
        error_msg = f'Failed to create D-ID stream: {str(e)}'
        print(f"[ERROR] {error_msg}")
        return jsonify({'error': error_msg}), 500
...
def did_stream_ice():
    ...
    try:
        ...
        if did_response.status_code == 200:
            ...
            print(f"[ERROR] D-ID ICE failed: {did_response.status_code} - {did_response.text}")
            return jsonify({'error': f'ICE candidate failed: {did_response.text}'}), 500
...
def did_stream_sdp():
    ...
    try:
        ...
        if did_response.status_code == 200:
            ...
            print(f"[ERROR] D-ID SDP failed: {did_response.status_code} - {did_response.text}")
            return jsonify({'error': f'SDP processing failed: {did_response.text}'}), 500
...
def did_stream_talk():
    ...
    try:
        print(f"[DEBUG] Sending talk request: {payload}")
        ...
        print(f"[DEBUG] Talk response status: {did_response.status_code}")
        print(f"[DEBUG] Talk response text: {did_response.text}")
        
        if did_response.status_code == 200:
            return jsonify(did_response.json())
        else:
            error_msg = f"D-ID talk failed: {did_response.status_code} - {did_response.text}"
            print(f"[ERROR] {error_msg}")
            return jsonify({'error': error_msg}), 500
            
    except requests.exceptions.RequestException as e:
        error_msg = f'Failed to send text to D-ID: {str(e)}'
        print(f"[ERROR] {error_msg}")
        return jsonify({'error': error_msg}), 500
...
if __name__ == '__main__':
    print("🚀 Starting AI Mock Interview Backend...")
